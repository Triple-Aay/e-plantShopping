...
function ProductList({ onHomeClick }) {
  ...
  const totalCartQty = cartItems.reduce((sum, it) => sum + it.quantity, 0);
  ...
  useEffect(() => {
    const next = {};
    for (const it of cartItems) next[it.name] = true;
    setAddedToCart(next);
  }, [cartItems]);
      ...
      {!showCart ? (
        <div className="product-grid">
          {plantsArray.map((categoryObj, index) => (
            <section className="category-block" key={index}>
                ...
                {categoryObj.plants.map((plant, plantIndex) => (
                  <li key={plantIndex} className="product-card">
                    ...
                    <button
                      ...
                      disabled={!!addedToCart[plant.name]} // Disable button if the product is already added
                    >
                      {addedToCart[plant.name]
                        ? "Added To Cart"
                        : "Add to Cart"}
                      {/* Change button text based on whether the product is added */}
                    </button>
                  </li>
                ))}
              </ul>
